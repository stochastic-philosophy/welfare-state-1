...
export class Router {
  // ... koko luokan sisältö on täsmälleen sama kuin aiemmin ...
  constructor(options = {}) {
    this.routes = [];
  ...
  resolve() {
    ...
    for (const route of this.routes) {
        ...
        if (match) {
            ...
            if (!getControllerInstance) { /* ... error ... */ return; }
            ...
            if (typeof action === 'function') {
                ...
                if (result && typeof result.then === 'function') {
                    result.then(html => { if (html) this.render(html); });
                } else {
  ...
  render(html) { this.rootElement.innerHTML = html; }
  navigate(hash) { window.location.hash = hash.startsWith('#') ? hash : '#' + hash; }
  setNotFoundHandler(handler) { this.notFoundHandler = handler; }
}
